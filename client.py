...
logging.basicConfig(filename=f'/logs/client1-{datetime.today().strftime("%Y-%m-%d")}.log', format = fmt_str, datefmt = fmt_date_str,level=logging.INFO)
...
for request in range(500):
    logging.info("Sending request %s message: %s", request,"Hello1")
